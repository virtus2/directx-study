...
int Input::Initialize(HWND hWnd)
{
...
void Input::Update()
{
    ...
    // TODO: 게임패드와 키보드 입력을 따로 분리해서 둘 중에 하나만 처리해야할 수도 있습니다.
    // 게임패드 입력 처리
    XINPUT_STATE newInputState;
    if (XInputGetState(0, &newInputState) == ERROR_SUCCESS)
    {
        ...
        digitalInputs[0][Pad_LeftTrigger] = newInputState.Gamepad.bLeftTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD;
        digitalInputs[0][Pad_RightTrigger] = newInputState.Gamepad.bRightTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD;
    ...
    {
		digitalInputs[0][i] = (keyBuffer[directXKeyMapping[i]] & 0x80) != 0;
	}

    for (uint32_t i = 0; i < 8; ++i)
    {
        if (mouseState.rgbButtons[i] > 0) digitalInputs[0][Mouse_0 + i] = true;
    }

    analogInputs[Mouse_X] = static_cast<float>(mouseState.lX * 0.0018f);
    analogInputs[Mouse_Y] = static_cast<float>(mouseState.lY * -0.0018f);

    if (mouseState.lZ > 0)
    {
        analogInputs[Mouse_Scroll] = 1.0f;
    }
...
{
    return digitalInputs[0] != 0;
}

bool Input::IsPressed(DigitalInput input)
{
    return digitalInputs[0][input];
}

bool Input::IsFirstPressed(DigitalInput input)
{
    return digitalInputs[0][input] && !digitalInputs[1][input];
}

bool Input::IsReleased(DigitalInput input)
{
    return !digitalInputs[0][input];
}

bool Input::IsFirstReleased(DigitalInput input)
{
    return !digitalInputs[0][input] && digitalInputs[1][input];
}

float Input::FilterAnalogInput(float input, float deadZone)
{
    if (input < 0)
    {
        if (input > -deadZone)
            ...
            return (input + deadZone) / (32768.0f - deadZone);
    }
    else
    {
        if (input < deadZone)
            ...
            return (input - deadZone) / (32767.0f - deadZone);
    }
}

float Input::GetAnalogInput(AnalogInput input)
{
    return analogInputs[input];
}
