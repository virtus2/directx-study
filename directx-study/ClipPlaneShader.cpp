...
ClipPlaneShader::ClipPlaneShader(const ClipPlaneShader&)
{
}
...
bool ClipPlaneShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, 
...
bool ClipPlaneShader::InitializeShader(ID3D11Device* device, HWND hWnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&clipPlaneBufferDesc, NULL, &clipPlaneBuffer);
	if (FAILED(result))
	{
		return false;
	}
...
bool ClipPlaneShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, 
	ID3D11ShaderResourceView* texture, XMFLOAT4 clipPlane)
{
	HRESULT result;
	...
	result = deviceContext->Map(clipPlaneBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}
	...
	// Unlock the buffer.
	deviceContext->Unmap(clipPlaneBuffer, 0);
	...
	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &clipPlaneBuffer);
